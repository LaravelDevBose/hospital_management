...
class Brand extends CI_Model
{
	...
	protected $column = 'brand_id,brand_code,brand_name,brand_status, created_at';

	protected $id = 'brand_id';
	protected $code = 'brand_code';
	protected $name = 'brand_name';
	protected $status = 'brand_status';
	...
	public function _create_code(){
		...
		if(is_null($code_info)|| !isset($code_info)){
			...
			$num = substr($code_info->brand_code, 1, strlen($code_info->brand_code));

			if($num < 9):
				$num+=1;
				$generateCode = 'B00'.$num;
			elseif($num < 99):
				$num+=1;
				$generateCode = 'B0'.$num;
			else:
